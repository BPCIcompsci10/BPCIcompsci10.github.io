...
let URL = "https://teachablemachine.withgoogle.com/models/UimsjzlEP/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.6 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0.5 && time <= 5.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 11.0 && time <= 15.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
                 break;
            case '3':
                if (time >= 29.0 && time <= 33.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 12.0 && time <= 16.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 40.0 && time <= 44.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/UimsjzlEP/");
    }
}
