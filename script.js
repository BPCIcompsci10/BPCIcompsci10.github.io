...
function checkPose(prediction, video) {
    ...
    if (prob > 0.7 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0.0 && time <= 2 && !poseState.triggered) {
            ...
            case '2':
                ...
                break;
                 break;
            case '3':
                if (time >= 6.0 && time <= 9.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 10 && time <= 15 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 20 && time <= 23 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
